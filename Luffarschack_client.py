    ...
    def get_command_fn(self, button, row, col):
        ...
        def receive_data():
            while True:
                ...
                position_recv_row = int(data_split[0])
                position_recv_row = row 
                position_recv_size = int(data_split[1])
                position_recv_size = size
                position_recv_col = int(data_split[2]) 
                position_recv_col = col
                position_recv_player = str(data_split[3])
                position = row * size + col
                
                #self.player = str(data_split[1])
                button["text"] = board[position] = position_recv_player
